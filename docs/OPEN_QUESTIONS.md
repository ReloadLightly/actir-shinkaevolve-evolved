...
| 7 | Judge model unspecified (explicitly an M0 decision, §8). | Proposed `claude-haiku-4-5-20251001`, temperature 0. See `JUDGE_MODEL_NOTE.md` — the "temperature 0" requirement is what drives this. | `configs/judge.yaml` |
| 8 | The three scenario texts and the rubric are yours to write (§8). | Wrote real drafts (~250 words each) rather than lorem ipsum, so the M1 smoke test has something to exercise. **All four are marked DRAFT and are not frozen.** | Rewrite freely, then `python scripts/freeze.py --version <new>` |
...
1. **Stage B authorization and an API key.** `configs/judge.yaml` ships with
   ...
   call. I will not change either without you saying so in chat.
2. **Approval of the scenario texts and the rubric (M0).** They are drafts. I
   wrote them from the design's one-line descriptions; they encode judgment
   calls about what each world contains that are properly yours.
3. **The feasibility bound.** 3.5% of GDP is the design's own example. It is
   the single most consequential number in the gate — it bounds how far
   rearmament portfolios can go.
4. **The four rival-school seeds for M1** (autonomous rearmament;
   accommodation; status-quo-plus; middle-power internationalism). Stage B
   work; not written yet.
